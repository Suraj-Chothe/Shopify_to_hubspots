...
def create_or_update_hubspot_contact(email, properties):
    ...
    url_update = f"https://api.hubapi.com/crm/v3/objects/contacts/email/{email}"
    headers = {
        "Authorization": f"Bearer {HUBSPOT_ACCESS_TOKEN}",
        "Content-Type": "application/json",
    }
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            # Contact already exists, attempt to update it
            print(f"ℹ️ Contact already exists: {email}. Attempting update…")
            response = httpx.patch(url_update, headers=headers, json=payload, timeout=30)
            if response.status_code == 200:
                print(f"🔄 Updated contact: {email}")
            elif response.status_code in (409, 412):
                # Conflict/precondition failures often mean no actual changes to apply
                print(f"✅ Contact already exists and is up to date: {email}")
            else:
                print(f"❌ Could not apply updates for {email}: {response.status_code} {response.text}")
        else:
